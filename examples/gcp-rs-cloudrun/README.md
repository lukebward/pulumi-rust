[![Deploy](https://get.pulumi.com/new/button.svg)](https://app.pulumi.com/new?template=https://github.com/lukebward/pulumi-rust/tree/main/examples/gcp-rs-cloudrun)
...
1.  Generate the GCP provider SDK into `./sdks`:
    ...
    The version is pinned deliberately. `ServiceArgs`,
    `CloudrunServiceTemplateSpecContainerArgs`, and `IamMemberArgs` each have
    at least one required input (`location`, `image`, and
    `service`/`role`/`member` respectively), so the generator does not derive
    `Default` for any of them and `src/main.rs` names every field explicitly
