...
1.  Generate the azure-native provider SDK into `./sdks`:
    ...
    The version is pinned deliberately. Every azure-native resource has a
    required `resourceGroupName` input, so the generator derives `Default`
    for none of the args structs here and `src/main.rs` names every field
