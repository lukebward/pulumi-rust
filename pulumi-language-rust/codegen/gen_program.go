...
func GenerateProgram(program *pcl.Program) (map[string][]byte, hcl.Diagnostics, error) {
	...
	for _, dir := range dirs {
		module := componentModuleName(dir)
		for used[module] {
			module += "_"
		}
		used[module] = true
	...
	g.componentModules = modules
	for _, dir := range dirs {
		source, cdiags := generateComponentModule(components[dir], modules)
		diags = append(diags, cdiags...)
		files["src/"+modules[dir]+".rs"] = source
	}

	source, mdiags := g.generate()
...
// generateComponentModule emits the Rust module implementing one component.
func generateComponentModule(
	component *pcl.Component, modules map[string]string,
) ([]byte, hcl.Diagnostics) {
	g := newProgramGenerator(component.Program)
	g.isComponent = true
	g.componentModules = modules
...
type programGenerator struct {
	...
	// Component mode: the generator is emitting a component module rather
	// than the program's main function.
	isComponent bool
	...
	// componentVars records which Rust variables hold component instances.
	componentVars map[string]*pcl.Component
...
// writeComponentModule renders the struct, args and constructor for a
// component whose body has already been generated.
func (g *programGenerator) writeComponentModule(w *bytes.Buffer, body *bytes.Buffer) {
	typeName := componentTypeName(g.component)
	var configs []*pcl.ConfigVariable
	for _, n := range g.component.Program.Nodes {
		if c, ok := n.(*pcl.ConfigVariable); ok {
	...
	fmt.Fprintf(w, "    /// component's own registration to break the cycle.\n")
	fmt.Fprintf(w, "    pub pulumi_deferred: Vec<String>,\n")
	fmt.Fprintf(w, "}\n\n")
	fmt.Fprintf(w, "impl Default for %sArgs {\n", typeName)
	...
	fmt.Fprintf(w, "    pub async fn new(ctx: &pulumi::Context, name: &str, args: %sArgs, options: pulumi::ResourceOptions) -> pulumi::Result<%s> {\n",
		typeName, typeName)
	fmt.Fprintf(w, "        let ctx = pulumi::Context::clone(ctx);\n")
	...
	fmt.Fprintf(w, "        __deferred.extend(args.pulumi_deferred.iter().cloned());\n")
	fmt.Fprintf(w, "        let __component = ctx.register_resource(pulumi::RegisterRequest {\n")
	fmt.Fprintf(w, "            type_: %s.to_string(),\n", rustString(componentToken(g.component)))
	...
	fmt.Fprintf(w, "        ctx.register_resource_outputs(&__component, vec![%s]);\n",
		strings.Join(g.componentOutputs, ", "))
	fmt.Fprintf(w, "        Ok(%s {\n", typeName)
...
func (g *programGenerator) genConfigVariable(w *bytes.Buffer, cfg *pcl.ConfigVariable) {
	key := cfg.LogicalName()
	// A component's config comes from its arguments, not stack config.
	if g.isComponent {
		...
		if cfg.DefaultValue != nil {
			...
			fallback = g.expr(converted)
		}
		fmt.Fprintf(w, "let %s = args.%s.clone().unwrap_or_else(|| %s);\n", name, field, fallback)
		g.componentArgs[cfg.Name()] = true
		return
	}
	kind := "object"
...
func (g *programGenerator) genOutputVariable(w *bytes.Buffer, output *pcl.OutputVariable) {
	if g.isComponent {
		// A component publishes its outputs with RegisterResourceOutputs.
		i := len(g.componentOutputs)
...
// component every child is prefixed with the component instance's name.
func (g *programGenerator) resourceName(logicalName string) string {
	if g.isComponent {
		return fmt.Sprintf("&format!(\"{}-%s\", name)", logicalName)
	}
	return rustString(logicalName)
...
func (g *programGenerator) genResource(w *bytes.Buffer, r *pcl.Resource) {
	...
	fmt.Fprintf(w, "let %s = %s::new(&ctx, %s, %s, %s);\n",
		name, structPath, g.resourceName(r.LogicalName()), args, options)
}
...
func (g *programGenerator) genStackReference(w *bytes.Buffer, r *pcl.Resource) {
	...
	fmt.Fprintf(w, "let %s = pulumi::StackReference::new(&ctx, %s, %s, %s);\n",
		name, g.resourceName(r.LogicalName()), nameExpr, g.resourceOptions(r))
}
...
// componentTypeName is the Rust type name for a component declaration.
func componentTypeName(c *pcl.Component) string {
...
func (g *programGenerator) genComponent(w *bytes.Buffer, c *pcl.Component) {
	subject := c.Definition.Syntax.DefRange()
	module, ok := g.componentModules[c.DirPath()]
	if !ok {
		g.errorf(subject, "unknown component directory %q", c.DirPath())
		return
	}
	typeName := componentTypeName(c)
	path := "crate::" + module + "::" + typeName
	if g.isComponent {
		path = "crate::" + module + "::" + typeName
	}
...
func (g *programGenerator) componentOptions(c *pcl.Component, subject hcl.Range) string {
	if c.Options == nil {
		if g.isComponent {
	...
	return g.optionsLiteral(c.Options, subject)
}
...
func (g *programGenerator) resourceOptions(r *pcl.Resource) string {
	...
	return g.optionsLiteral(r.Options, r.Definition.Syntax.DefRange())
}
...
func (g *programGenerator) optionsLiteral(opts *pcl.ResourceOptions, subject hcl.Range) string {
	...
	} else if g.isComponent {
		// Inside a component, anything without an explicit parent belongs
		// to the component.
		setField("parent", "Some(__component.clone())")
	}
...
func (g *programGenerator) resourceRefNode(expr model.Expression) (string, *pcl.Resource, bool) {
	...
	if len(scope.Parts) > 0 {
		...
		if _, ok := scope.Parts[0].(*pcl.Component); ok {
			return g.refVar(scope.RootName), nil, true
		}
	}
	return "", nil, false
}
...
func (g *programGenerator) scopeTraversalExpr(expr *model.ScopeTraversalExpression) string {
	...
	case *pcl.Component:
		...
		attr, ok := rest[0].(hcl.TraverseAttr)
		if !ok {
			g.errorf(expr.SyntaxNode().Range(), "unsupported component traversal")
			return "pulumi::pv::null()"
		}
		if attr.Name == "urn" {
...
func (g *programGenerator) invokeExpr(expr *model.FunctionCallExpression) string {
	...
	options := g.defaultInvokeOptions()
	if len(expr.Args) > 2 {
...
// defaultInvokeOptions parents invokes inside a component to that component,
// so they resolve the providers the component was given.
func (g *programGenerator) defaultInvokeOptions() string {
	if g.isComponent {
		return "pulumi::InvokeOptions { parent: Some(__component.clone()), ..Default::default() }"
	}
	return "pulumi::InvokeOptions::default()"
}
