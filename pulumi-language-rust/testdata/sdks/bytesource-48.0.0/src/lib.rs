...
pub struct ResourceArgs {
    pub base64: Option<pulumi::Output<std::string::String>>,
}
...
impl ResourceArgs {
    pub fn into_inputs(self) -> std::vec::Vec<(std::string::String, pulumi::Output<pulumi::PropertyValue>)> {
        ...
        if let Some(v) = self.base64 {
            inputs.push(("base64".to_string(), v.cast()));
        }
        inputs
    }
...
impl Resource {
    pub fn new(ctx: &pulumi::Context, name: &str, args: ResourceArgs, options: pulumi::ResourceOptions) -> Resource {
        let resource = ctx.register_resource(pulumi::RegisterRequest {
            type_: "bytesource:index:Resource".to_string(),
            name: name.to_string(),
            custom: true,
            remote: false,
            version: "48.0.0".to_string(),
    ...
    pub fn base64(&self) -> pulumi::Output<std::string::String> {
        self.resource.output("base64").cast()
    }
...
impl Provider {
    pub fn new(ctx: &pulumi::Context, name: &str, args: ProviderArgs, options: pulumi::ResourceOptions) -> Provider {
        let resource = ctx.register_resource(pulumi::RegisterRequest {
            type_: "pulumi:providers:bytesource".to_string(),
            name: name.to_string(),
            custom: true,
            remote: false,
            version: "48.0.0".to_string(),
