...
pub struct ResourceArgs {
    pub bytes: Option<pulumi::Output<std::string::String>>,
    pub expect_base64: Option<pulumi::Output<std::string::String>>,
}
...
impl ResourceArgs {
    pub fn into_inputs(self) -> std::vec::Vec<(std::string::String, pulumi::Output<pulumi::PropertyValue>)> {
        ...
        if let Some(v) = self.bytes {
            inputs.push(("bytes".to_string(), v.cast()));
        }
        if let Some(v) = self.expect_base64 {
            inputs.push(("expectBase64".to_string(), v.cast()));
        }
        inputs
    }
...
impl Resource {
    pub fn new(ctx: &pulumi::Context, name: &str, args: ResourceArgs, options: pulumi::ResourceOptions) -> Resource {
        let resource = ctx.register_resource(pulumi::RegisterRequest {
            type_: "bytesink:index:Resource".to_string(),
            name: name.to_string(),
            custom: true,
            remote: false,
            version: "47.0.0".to_string(),
            plugin_download_url: "".to_string(),
            inputs: args.into_inputs(),
            options,
            package: None,
            deferred_inputs: vec![],
            required: &["bytes", "expectBase64"],
        });
        Resource { resource }
    }
    ...
    pub fn bytes(&self) -> pulumi::Output<std::string::String> {
        self.resource.output("bytes").cast()
    }

    pub fn expect_base64(&self) -> pulumi::Output<std::string::String> {
        self.resource.output("expectBase64").cast()
    }
}
...
impl Provider {
    pub fn new(ctx: &pulumi::Context, name: &str, args: ProviderArgs, options: pulumi::ResourceOptions) -> Provider {
        let resource = ctx.register_resource(pulumi::RegisterRequest {
            type_: "pulumi:providers:bytesink".to_string(),
            name: name.to_string(),
            custom: true,
            remote: false,
            version: "47.0.0".to_string(),
